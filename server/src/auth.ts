  ...
  try {
    ...
    if (identityImage?.dataUrl) {
      try {
        ...
        console.warn('Face embedding extraction failed, continuing without face verification:', (faceError as Error).message);
        // Continue signup without face embedding if local AI fails
      }
    }
