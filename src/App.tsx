...
import { AlertTriangle, ArrowLeft, ArrowRight, Camera, CheckCircle2, Clock, IdCard, KeyRound, LoaderCircle, LockKeyhole, MapPin, QrCode, ScanLine, ShieldCheck, Smartphone, Users } from 'lucide-react'
  ...
  return <main className="login-page">
    ...
    <section className="login-form-side">
      ...
      <form className="login-form" onSubmit={resetMode ? submitReset : submitLogin}>
        ...
        {!supabase && <div className="demo-notice"><CheckCircle2 size={16}/><span>Demo mode is active because Supabase environment values are not configured.</span></div>}
        </>}
      </form>
...
export function App() {
    ...
    async function checkDeveloperMode() {
      if (Capacitor.isNativePlatform() && Capacitor.getPlatform() === 'android') {
        try {
          ...
          const DeveloperMode = registerPlugin<{ isDeveloperModeEnabled(): Promise<{enabled: boolean}> }>('DeveloperMode')
          const { enabled } = await DeveloperMode.isDeveloperModeEnabled()
          if (enabled) {
            setDeveloperModeBlocked(true)
          }
        } catch (e) {
  ...
  if (developerModeBlocked) {
    return (
      <div style={{ padding: 40, textAlign: 'center', height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#09090b', color: '#fff' }}>
        <ShieldCheck size={48} color="#ef4444" style={{ marginBottom: 20 }} />
        <h2>Security Alert</h2>
        <p style={{ color: '#a1a1aa', marginTop: 10 }}>Please turn off Developer Mode in your device settings to use this app.</p>
      </div>
    )
  }
