...
import { Bell, BookOpen, CalendarDays, ChevronDown, CircleUserRound, ClipboardCheck, Grid2X2, History, KeyRound, LogOut, Menu, QrCode, Server, Settings, ShieldCheck, Smartphone, Users, X } from 'lucide-react'
import { useState, type ReactNode } from 'react'
import type { Role } from '../types'
import { useAppStore } from '../store'
import { Initials, Logo, Button } from './ui'
import { getLocalServerUrl, setLocalServerUrl, isSupabaseConfigured } from '../lib/supabase'
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${menuOpen ? 'sidebar-open' : ''}`}>
      ...
      <div className="sidebar-foot">
        ...
        <a
          ...
          className="signout"
          style={{ textDecoration: 'none', color: '#10b981', borderColor: 'rgba(16, 185, 129, 0.25)', marginBottom: '0.4rem', justifyContent: 'center' }}
          title="Download latest Android APK release from GitHub"
        >
    ...
    <main className="workspace">
      <header className="topbar">
        ...
        <button
          className="role-chip"
          style={{ cursor: 'pointer', background: 'rgba(59, 130, 246, 0.12)', color: '#60a5fa', border: '1px solid rgba(59, 130, 246, 0.3)', display: 'flex', alignItems: 'center', gap: '0.4rem' }}
          onClick={() => setServerModalOpen(true)}
          title="Configure Server URL"
        >
          <Server size={14} />
          <span>{isSupabaseConfigured ? 'Supabase' : 'Local Server'}</span>
        </button>
      ...
      <div className="modal-backdrop" onClick={() => setServerModalOpen(false)}>
        <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '440px', padding: '1.5rem' }}>
          ...
          <p style={{ fontSize: '0.88rem', color: 'var(--text-muted, #94a3b8)', marginBottom: '1rem' }}>
            When running AttendX on your phone or on Vercel, enter your laptop's IP address (e.g. <code>http://192.168.1.50:3001</code>) so the app can communicate with your local database.
          </p>
          <input
            type="text"
            className="input"
